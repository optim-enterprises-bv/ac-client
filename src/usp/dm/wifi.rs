...
pub async fn get(_cfg: &ClientConfig, path: &str) -> HashMap<String, String> {
    ...
    if path.contains("SSID.") || path.ends_with("Device.WiFi.") {
        for (idx, iface) in ifaces.iter().enumerate() {
            ...
            if !ssid.is_empty() {
                ...
                m.insert(
                    format!("Device.WiFi.SSID.{ssid_idx}.Enable"),
                    enable.to_string(),
                );
                // SSID Status: Up if enabled
                m.insert(
                    format!("Device.WiFi.SSID.{ssid_idx}.Status"),
                    if enable { "Up" } else { "Down" }.to_string(),
                );
    ...
    if path.contains("AccessPoint.") || path.ends_with("Device.WiFi.") {
        for (idx, iface) in ifaces.iter().enumerate() {
            ...
            m.insert(
                format!("Device.WiFi.AccessPoint.{ap_idx}.Security.ModeEnabled"),
                mode_friendly.to_string(),
            );
            if !key.is_empty() {
                m.insert(
                    format!("Device.WiFi.AccessPoint.{ap_idx}.Security.KeyPassphrase"),
                    key,
                );
            }
            if !mode.is_empty() {
            ...
            let ap_phy = {
                ...
                if !ubus_iface.is_empty() {
                    ...
                    if !ifname.is_empty() {
                        ifname
                    } else if !ap_device.is_empty() {
                        get_phy_interface(&ap_device)
                    } else {
                        String::new()
                    }
                }
            ...
            m.insert(
                format!("Device.WiFi.AccessPoint.{ap_idx}.SSIDAdvertisementEnabled"),
            ...
            m.insert(
                format!("Device.WiFi.AccessPoint.{ap_idx}.Security.MFPConfig"),
                mfp.to_string(),
            );
            ...
            if !phy_iface.is_empty() {
                ...
                m.insert(
                    format!("Device.WiFi.AccessPoint.{ap_idx}.AssociatedDeviceNumberOfEntries"),
                    count.to_string(),
                );
            } else {
                m.insert(
    ...
    if path.contains("Radio.") || path.ends_with("Device.WiFi.") {
        for (idx, device) in devices.iter().enumerate() {
            ...
            if !band.is_empty() {
                ...
                m.insert(
                    format!("Device.WiFi.Radio.{radio_idx}.OperatingFrequencyBand"),
                    band_friendly.to_string(),
                );
            }
            if !htmode.is_empty() {
                m.insert(
            ...
            if !txpower.is_empty() {
                m.insert(
                    format!("Device.WiFi.Radio.{radio_idx}.TransmitPower"),
                    txpower,
                );
            }
            if !beacon_int.is_empty() {
                m.insert(
                    format!("Device.WiFi.Radio.{radio_idx}.BeaconPeriod"),
                    beacon_int,
                );
            }
            if !dtim_period.is_empty() {
                m.insert(
                    format!("Device.WiFi.Radio.{radio_idx}.DTIMPeriod"),
                    dtim_period,
                );
            }
            ...
            if !rts_threshold.is_empty() {
                m.insert(
                    format!("Device.WiFi.Radio.{radio_idx}.RTSThreshold"),
                    rts_threshold,
                );
            } else {
                m.insert(
            ...
            m.insert(
                format!("Device.WiFi.Radio.{radio_idx}.GuardInterval"),
                gi_value.to_string(),
            );
            ...
            if !max_assoc.is_empty() {
                m.insert(
                    format!("Device.WiFi.Radio.{radio_idx}.MaxAssociatedDevices"),
                    max_assoc,
                );
            } else {
                m.insert(
            ...
            let status = if enable && !phy_iface.is_empty() {
                "Up"
            } else {
                "Down"
            };
            m.insert(
                format!("Device.WiFi.Radio.{radio_idx}.Status"),
                status.to_string(),
            );
            ...
            if !radio_bssid.is_empty() {
                m.insert(
                    format!("Device.WiFi.Radio.{radio_idx}.X_OptimACS_BSSID"),
                    radio_bssid,
                );
            }
            ...
            if !phy_iface.is_empty() {
                ...
                if !bitrate.is_empty() {
                    m.insert(
                        format!("Device.WiFi.Radio.{radio_idx}.X_OptimACS_Bitrate"),
                        bitrate,
                    );
                }
    ...
    if path.contains("AccessPoint.") || path.ends_with("Device.WiFi.") {
        for (idx, iface) in ifaces.iter().enumerate() {
            ...
            if !maxassoc.is_empty() {
                m.insert(
                    format!("Device.WiFi.AccessPoint.{ap_idx}.MaxAssociatedDevices"),
                    maxassoc,
                );
            }
            let wmm_enabled = wmm != "0";
            m.insert(
                format!("Device.WiFi.AccessPoint.{ap_idx}.WMMEnable"),
                wmm_enabled.to_string(),
            );
            let isolation = isolate == "1";
            m.insert(
                format!("Device.WiFi.AccessPoint.{ap_idx}.IsolationEnable"),
                isolation.to_string(),
            );
        }
    }
...
fn get_phy_interface(device: &str) -> String {
    ...
    if iface_name.is_empty() {
        let idx = device
            ...
            .parse::<usize>()
            .unwrap_or(0);
        for candidate in &[format!("phy{idx}-ap0"), format!("wlan{idx}")] {
                    ...
                    "tx bitrate" => {
                        ...
                        if let Some(rate_str) = val.split_whitespace().next() {
                            if let Ok(mbps) = rate_str.parse::<f64>() {
                                sta.insert(
                                    "tx_bitrate".to_string(),
                                    format!("{}", (mbps * 1000.0) as u64),
                                );
                            } else {
    ...
    if path.ends_with(".SSID") {
        if let Some(idx) = parse_ssid_index(path) {
            if idx > 0 && idx <= ifaces.len() {
                ...
                return Err(format!(
                    "SSID index {idx} out of range (max: {})",
                    ifaces.len()
                ));
            }
        }
    ...
    // Handle SSID Enable
        ...
        if let Some(idx) = parse_ssid_index(path) {
            if idx > 0 && idx <= ifaces.len() {
                ...
                let disabled = if value == "true" || value == "1" {
                    "0"
                } else {
                    "1"
                };
    ...
    // Handle SSID Advertisement (hidden SSID)
        ...
        if let Some(idx) = parse_ssid_index(path) {
            if idx > 0 && idx <= ifaces.len() {
                ...
                let hidden = if value == "true" || value == "1" {
                    "0"
                } else {
                    "1"
                };
    ...
    // Handle Radio Channel
        ...
        if let Some(idx) = parse_radio_index(path) {
            if idx > 0 && idx <= devices.len() {
                ...
                return Err(format!(
                    "Radio index {idx} out of range (max: {})",
                    devices.len()
                ));
            }
        }
