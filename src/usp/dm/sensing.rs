//! Firewall-drop observations, outbound — `Device.X_OptimACS_Sensing.*`.
//!
//! The third courier, alongside `dm::enforcement` (proof out) and
//! `dm::reputation` (feed in). This one carries what our own firewall stopped,
//! which is the observation the whole reputation design is actually for: a
//! public feed cannot corroborate itself, and a fleet can.
//!
//! # Consent
//!
//! Sensing is **off by default** in `aether-sensord` and consented separately
//! from everything else it does, because it reports attacker addresses and
//! those are personal data. Nothing here can switch it on. A device that has
//! not opted in has no spool, this module reports nothing, and the controller
//! sees a quiet sensor — which is true, not a fault.
//!
...
/// Where `aether-sensord` writes batches. Matches its `sense_spool` default.
const SPOOL: &str = "/var/spool/aether-sensord/sense";
...
/// Batch files are `batch-<unix seconds>.ndjson`. `.partial` files are excluded
...
pub fn take_batches() -> Option<String> {
    ...
    for (path, _) in found.into_iter().take(MAX_BATCHES_PER_GET) {
        let Ok(text) = fs::read_to_string(&path) else {
            warn!("sensing: cannot read {}, leaving it", path.display());
            continue;
        };
        if !body.is_empty() && body.len() + text.len() > MAX_BYTES {
            ...
            // against the sensor.
            debug!("sensing: size ceiling reached, deferring the rest");
            break;
        }
        body.push_str(&text);
        if !body.ends_with('\n') {
    ...
    for p in &taken {
        if let Err(e) = fs::remove_file(p) {
            warn!(
                "sensing: shipped {} but could not remove it: {e} -- it will be \
    ...
    info!(
        "sensing: reporting {} batch(es), {} bytes",
        taken.len(),
        body.len()
    );
    Some(body)
...
pub fn get(_cfg: &ClientConfig, path: &str) -> HashMap<String, String> {
    ...
    m.insert(
        "Device.X_OptimACS_Sensing.PendingBatches".into(),
        waiting.to_string(),
    );

    if let Some(body) = take_batches() {
        m.insert("Device.X_OptimACS_Sensing.Observations".into(), body);
    }
    m
}
...
mod tests {
    ...
    fn tempdir(tag: &str) -> PathBuf {
        let p = std::env::temp_dir().join(format!("ac-sensing-{}-{tag}", std::process::id()));
    ...
    fn only_complete_batches_are_candidates() {
        assert_eq!(
            timestamp_of(Path::new("/x/batch-1787461200.ndjson")),
            Some(1787461200)
        );
        ...
        // the sensor.
        assert_eq!(
            timestamp_of(Path::new("/x/batch-1787461200.ndjson.partial")),
            None
        );
        assert_eq!(timestamp_of(Path::new("/x/canary-1787461200.ndjson")), None);
    }
    ...
    fn batches_come_out_oldest_first() {
        ...
        for t in [1787461300u64, 1787461100, 1787461200] {
            fs::write(dir.join(format!("batch-{t}.ndjson")), "{}\n").unwrap();
        }
