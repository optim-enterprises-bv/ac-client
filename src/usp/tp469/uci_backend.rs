...
pub fn add_dhcp_lease(mac: &str, ip: &str, hostname: Option<&str>) -> UciResult {
    info!("Adding DHCP static lease: MAC={}, IP={}, Host={:?}", mac, ip, hostname);
    
    // Find next available host section index
    let next_idx = find_next_dhcp_host_index();
    if next_idx == 0 {
        return UciResult::error(ErrorCode::ResourcesExceeded, "Could not find available host section");
    }
    ...
    if let Err(e) = uci_add("dhcp", "host") {
        return UciResult::error(ErrorCode::InternalError, &format!("Failed to add host section: {}", e));
    }
    ...
    if let Err(e) = uci_set(&format!("dhcp.{}.mac", section), mac) {
        ...
        return UciResult::error(ErrorCode::InternalError, &format!("Failed to set MAC: {}", e));
    }
    ...
    if let Err(e) = uci_set(&format!("dhcp.{}.ip", section), ip) {
        // Rollback
        let _ = uci_delete(&format!("dhcp.{}", section));
        return UciResult::error(ErrorCode::InternalError, &format!("Failed to set IP: {}", e));
    }
    ...
    if let Err(e) = uci_commit("dhcp") {
        // Rollback
        let _ = uci_delete(&format!("dhcp.{}", section));
        return UciResult::error(ErrorCode::InternalError, &format!("Failed to commit: {}", e));
    }
    ...
    match out {
        Ok(result) if result.status.success() => {
            ...
            if let Err(e) = uci_delete(&full_path) {
                return UciResult::error(ErrorCode::InternalError, &format!("Failed to delete: {}", e));
            }
        ...
        _ => {
            UciResult::error(ErrorCode::ObjectNotFound, &format!("DHCP lease instance {} not found", instance))
        }
    }
}
...
pub fn add_wifi_interface(
    ...
    if next_idx == 0 {
        return UciResult::error(ErrorCode::ResourcesExceeded, "Could not find available wifi-iface section");
    }
    ...
    if let Err(e) = uci_add("wireless", "wifi-iface") {
        return UciResult::error(ErrorCode::InternalError, &format!("Failed to add wifi-iface: {}", e));
    }
    
    // Set SSID
    if let Err(e) = uci_set(&format!("wireless.{}.ssid", section), ssid) {
        let _ = uci_delete(&format!("wireless.{}", section));
        return UciResult::error(ErrorCode::InternalError, &format!("Failed to set SSID: {}", e));
    }
    ...
    if let Err(e) = uci_set(&format!("wireless.{}.device", section), radio_device) {
        let _ = uci_delete(&format!("wireless.{}", section));
        return UciResult::error(ErrorCode::InternalError, &format!("Failed to set device: {}", e));
    }
    
    // Set mode (default to ap)
    if let Err(e) = uci_set(&format!("wireless.{}.mode", section), "ap") {
        let _ = uci_delete(&format!("wireless.{}", section));
        return UciResult::error(ErrorCode::InternalError, &format!("Failed to set mode: {}", e));
    }
    
    // Set network (default to lan)
    if let Err(e) = uci_set(&format!("wireless.{}.network", section), "lan") {
        let _ = uci_delete(&format!("wireless.{}", section));
        return UciResult::error(ErrorCode::InternalError, &format!("Failed to set network: {}", e));
    }
...
pub fn delete_wifi_interface(instance: u32) -> UciResult {
    ...
    match out {
        ...
        _ => {
            UciResult::error(ErrorCode::ObjectNotFound, &format!("WiFi interface instance {} not found", instance))
        }
    }
}
...
pub fn add_static_host(ip: &str, hostname: &str) -> UciResult {
    ...
    if let Err(e) = uci_add_list("dhcp.@dnsmasq[0].address", &address_entry) {
        ...
        if let Err(e2) = add_to_hosts_file(ip, hostname) {
            return UciResult::error(ErrorCode::InternalError, 
...
pub fn delete_static_host(instance: u32) -> UciResult {
    ...
    if let Err(e) = remove_from_hosts_file(instance) {
        return UciResult::error(ErrorCode::ObjectNotFound, 
...
pub fn uci_set_value(config: &str, section: &str, option: &str, value: &str, auto_commit: bool) -> Result<(), String> {
    let path = format!("{}.{}.{}", config, section, option);
    uci_set(&path, value)?;
...
pub fn uci_add_to_list(config: &str, section: &str, option: &str, value: &str, auto_commit: bool) -> Result<(), String> {
    let path = format!("{}.{}.{}", config, section, option);
...
pub fn uci_remove_from_list(config: &str, section: &str, option: &str, value: &str, auto_commit: bool) -> Result<(), String> {
    let path = format!("{}.{}.{}", config, section, option);
...
fn wifi_reload() -> Result<(), String> {
    ...
    // Fallback
    let status = Command::new("/sbin/wifi")
        .status();
...
pub fn get_wifi_device(radio_name: &str) -> Result<WifiDeviceConfig, String> {
    ...
    Ok(WifiDeviceConfig {
        ...
        channel: uci_get_value("wireless", radio_name, "channel").unwrap_or_else(|_| "auto".to_string()),
        band: uci_get_value("wireless", radio_name, "band").unwrap_or_else(|_| "2g".to_string()),
        htmode: uci_get_value("wireless", radio_name, "htmode").unwrap_or_else(|_| "HT20".to_string()),
...
pub fn update_wifi_device_param(
    radio_name: &str,
    param: &str,
    value: &str,
) -> UciResult {
    ...
    info!("Successfully updated WiFi device {}: {} = {}", radio_name, param, value);
    UciResult::success(1)
}
...
pub fn update_wifi_interface_param(
    iface_name: &str,
    param: &str,
    value: &str,
) -> UciResult {
    info!("Updating WiFi interface {}: {} = {}", iface_name, param, value);
    ...
    info!("Successfully updated WiFi interface {}: {} = {}", iface_name, param, value);
    UciResult::success(1)
}
...
pub fn add_network_interface(
    ...
    if !test.is_empty() {
        return UciResult::error(ErrorCode::DuplicateUniqueKey,
...
pub fn update_network_interface_param(
    ...
    info!("Successfully updated network interface {}: {} = {}", name, param, value);
    UciResult::success(1)
}
...
pub fn add_bridge_port(device_name: &str, port: &str) -> UciResult {
    ...
    if dev_type != "bridge" {
        return UciResult::error(ErrorCode::InvalidValue,
...
pub fn remove_bridge_port(device_name: &str, port: &str) -> UciResult {
    ...
    info!("Successfully removed bridge port {} from {}", port, device_name);
    UciResult::success(1)
}
...
pub fn set_device_mac(device_name: &str, mac: &str) -> UciResult {
    ...
    info!("Successfully set MAC address for device {}: {}", device_name, mac);
    UciResult::success(1)
}
...
pub fn remove_dns_server(iface_name: &str, dns: &str) -> UciResult {
    ...
    info!("Successfully removed DNS server {} from {}", dns, iface_name);
    UciResult::success(1)
}
...
pub fn set_ipv6_prefix(iface_name: &str, prefix: &str) -> UciResult {
    ...
    info!("Successfully set IPv6 prefix for {}: {}", iface_name, prefix);
    UciResult::success(1)
}
...
pub fn set_system_log_size(size: &str) -> UciResult {
    ...
    let _ = Command::new("/etc/init.d/log")
        .arg("restart")
        .status();
...
pub fn set_dhcp_pool(
    iface_name: &str,
    start: u32,
    limit: u32,
    leasetime: &str,
) -> UciResult {
    info!("Setting DHCP pool for {}: start={}, limit={}, leasetime={}", 
...
pub fn add_led_config(
    ...
    loop {
        ...
        idx += 1;
        if idx > 20 {
            return UciResult::error(ErrorCode::ResourcesExceeded,
                "Too many LED configurations");
        }
    }
