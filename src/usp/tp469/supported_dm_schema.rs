...
pub fn build_data_model_schema() -> ObjectSchema {
    ...
    // Device.DeviceInfo
    root.children
        .insert("DeviceInfo".into(), build_device_info_schema());
    ...
    // Device.LocalAgent
    root.children
        .insert("LocalAgent".into(), build_local_agent_schema());

    // Device.X_OptimACS extensions
    root.children
        .insert("X_OptimACS".into(), build_optimacs_schema());
...
fn build_device_info_schema() -> ObjectSchema {
    ...
    // Standard TR-181 DeviceInfo.HostName for system hostname (maps to UCI system.@system[0].hostname)
    params.insert(
        "HostName".into(),
        ParameterSchema {
            name: "HostName".into(),
            ...
            description: "System hostname (maps to OpenWrt UCI system.@system[0].hostname)".into(),
        },
    );

    params.insert(
        "X_OptimACS_LoadAvg".into(),
        ParameterSchema {
...
fn build_wifi_schema() -> ObjectSchema {
    ...
    radio.parameters.insert(
        "OperatingChannelBandwidth".into(),
        ParameterSchema {
            name: "OperatingChannelBandwidth".into(),
            param_type: ParamType::String,
            access: Access::ReadWrite,
            description:
                "Channel bandwidth (20MHz, 40MHz, 80MHz, 160MHz, EHT20, EHT80, EHT320 for WiFi 7)"
                    .into(),
        },
    );

    radio.parameters.insert(
        "X_OptimACS_CellDensity".into(),
        ParameterSchema {
    ...
    access_point.parameters.insert(
        "Security.ModeEnabled".into(),
        ParameterSchema {
...
fn build_local_agent_schema() -> ObjectSchema {
    ...
    la.commands.push(CommandSchema {
        ...
        input_args: vec![ParameterSchema {
...
fn build_optimacs_schema() -> ObjectSchema {
    ...
    camera.commands.push(CommandSchema {
        ...
        input_args: vec![],
        output_args: vec![ParameterSchema {
            name: "ImageURL".into(),
            param_type: ParamType::String,
            access: Access::ReadOnly,
    ...
    // Security object for device password management
    let mut security = ObjectSchema {
        name: "Security".into(),
        is_multi_instance: false,
        description: "Device security settings".into(),
        unique_keys: vec![],
        parameters: HashMap::new(),
        children: HashMap::new(),
        commands: vec![],
    ...
    // DevicePassword parameter - maps to OpenWrt root password
    security.parameters.insert(
        "DevicePassword".into(),
        ParameterSchema {
            name: "DevicePassword".into(),
            param_type: ParamType::String,
            access: Access::ReadWrite,
            description: "Device root password (maps to OpenWrt /etc/shadow)".into(),
        },
    );

    // IssueCert() command for certificate provisioning
    security.commands.push(CommandSchema {
        name: "IssueCert()".into(),
        description: "Issue device certificates from step-ca".into(),
        input_args: vec![],
        output_args: vec![],
    });
    ...
    optimacs.children.insert("Security".into(), security);
...
pub fn find_parameter_schema<'a>(
    root: &'a ObjectSchema,
    path: &str,
